...
const SECRET_KEY = "mimo.apiKey";
const RECENT_TRANSPORT_SUMMARY_LIMIT = 25;
const RECENT_TRANSPORT_SUMMARY_STORAGE_PREFIX = "mimo.recentTransportSummaries";
...
const MIMO_USER_AGENT = "mimo-copilot-chat/0.1.1 VSCode";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("mimo.manage", () => mimoProvider.manage()),
    vscode.commands.registerCommand("mimo.diagnostics", () => mimoProvider.showDiagnostics()),
    vscode.commands.registerCommand("mimo.setApiKey", () => mimoProvider.setApiKey()),
    vscode.commands.registerCommand("mimo.modelPickerDiagnostics", () => showModelPickerDiagnostics()),
  );

  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("mimo.showUsageStatusBar")) {
        resetUsageStatusBar();
      }
      if (event.affectsConfiguration("mimo.experimentalContextIndicator")) {
        void syncExperimentalContextIndicator();
      }
    }),
  );
...
async function showThinkingEffortPicker(): Promise<void> {
  ...
  if (!choice) return;
  const cfg = vscode.workspace.getConfiguration("mimo.thinking");
